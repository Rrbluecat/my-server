...
değişken sunucu = ağ.sunucu_kur(görev(istek, yanıt) {
    ...
    // 1. STATİK DOSYALAR (Logo Eklendi)
    eğer (istek.url == "/stil.css") { yanıt.dosya_gönder("stil.css"); döndür; }
    değilse eğer (istek.url == "/logo.png") { yanıt.dosya_gönder("logo.png"); döndür; }

    // 2. ANA SAYFA (Link Listeleme Eklendi)
    değilse eğer (istek.url == "/") {
        ...
        yanıt.gönder(görsel.çiz("arayüz.html", {
            ...
            linkler: link_html,
            saat: sistem_saati // Hata veren 'zaman' yerine 'saat' kullandık
        }));
    }
    ...
    // 10. LİNK EKLEME (Yeni Özellik)
    değilse eğer (istek.url == "/link_ekle" && istek.method == "POST") {
        ağ.post_yakala(istek, görev(gelen) {
